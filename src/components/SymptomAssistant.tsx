...
export default function SymptomAssistant({ doctors, onSelectDoctor, user }: SymptomAssistantProps) {
      ...
      const response = await openai.chat.completions.create({
        ...
        messages: [
          { 
            ...
            - ВАЖНО: Если в списке несколько врачей одной специальности, выбирай одного из них СЛУЧАЙНЫМ ОБРАЗОМ, чтобы нагрузка распределялась честно. Не рекомендуй всегда одного и того же.
            
            ФОРМАТ ВЫВОДА:
            В самом конце твоего сообщения ОБЯЗАТЕЛЬНО добавь метку: [RECOMMEND: Имя Врача].
            Имя должно в точности совпадать с именем из предоставленного списка.`
          },
          { 
            role: 'user', 
  ...
  return (
    <div className="max-w-5xl mx-auto py-12 px-4 h-[800px] flex flex-col lg:flex-row gap-8">
      {/* Sidebar Info */}
      ...
      <div className="flex-1 flex flex-col bg-white rounded-[3rem] shadow-[0_50px_100px_-20px_rgba(0,0,0,0.1)] border border-slate-100 overflow-hidden">
        ...
        <div className="p-8 border-b border-slate-50 flex items-center justify-between bg-slate-50/30">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h3 className="text-xl font-black text-slate-900 tracking-tight">ИИ-Ассистент ММЦ</h3>
              <p className="text-xs font-black text-teal-600 uppercase tracking-widest">AI Diagnostics Support</p>
            </div>
          </div>
        </div>
        ...
        <div ref={scrollRef} className="flex-1 overflow-y-auto p-8 space-y-8 scroll-smooth">
                  ...
                  <div className="space-y-4">
                    <div className={`p-6 rounded-[2rem] shadow-xl ${
                      ...
                      {msg.role === 'assistant' && lastFailedMessage && i === messages.length - 1 && (
                        <button 
                          ...
                          className="mt-4 flex items-center gap-2 text-xs font-black text-teal-600 uppercase tracking-widest hover:text-teal-700 transition-colors"
                        >
                    ...
                    {msg.recommendation && (
                      <motion.div 
                        ...
                        whileHover={{ y: -5 }}
                        className="bg-slate-900 p-6 rounded-[2rem] text-white shadow-2xl flex items-center gap-6 group"
                      >
                        <img 
                        ...
                        <div className="flex-1">
                          <p className="text-[10px] font-black text-teal-400 uppercase tracking-widest mb-1">{msg.recommendation.specialty}</p>
                          <p className="text-lg font-black tracking-tight">{msg.recommendation.name}</p>
                        </div>
                        <motion.button 
                          whileHover={{ scale: 1.1 }}
                          whileTap={{ scale: 0.9 }}
                          onClick={() => onSelectDoctor(msg.recommendation!)}
                          className="p-4 bg-teal-600 text-white rounded-2xl hover:bg-teal-500 transition-all shadow-xl shadow-teal-900/40"
                        >
          ...
          {isLoading && (
            ...
            >
              <div className="bg-white p-6 rounded-[2rem] shadow-xl border border-slate-50 flex items-center gap-4">
                <div className="relative">
                  <Loader2 className="w-6 h-6 animate-spin text-teal-600" />
                  <div className="absolute inset-0 blur-md bg-teal-400/20 animate-pulse" />
                </div>
                <span className="text-sm font-black text-slate-500 uppercase tracking-widest">ИИ анализирует ваши симптомы...</span>
              </div>
            </motion.div>
          )}
        </div>
        ...
        <div className="p-8 bg-slate-50/50 border-t border-slate-100">
            ...
            <div className="mb-8 flex flex-wrap gap-3 justify-center">
              {quickQuestions.map((q, i) => (
                <motion.button
                  ...
                  className="px-6 py-3 bg-white border border-slate-100 rounded-2xl text-xs font-black text-slate-600 hover:border-teal-500 hover:text-teal-600 transition-all shadow-sm"
                >
                  {q}
                </motion.button>
              ))}
          ...
          <div className="relative flex items-center max-w-4xl mx-auto">
            <input 
              ...
              placeholder="Опишите ваши симптомы (например: болит голова и температура)..."
              className="w-full pl-8 pr-20 py-6 bg-white border-2 border-transparent focus:border-teal-500 rounded-[2rem] shadow-2xl outline-none transition-all font-bold text-slate-900 placeholder:text-slate-300"
            />
            <motion.button 
              ...
              className="absolute right-3 p-4 bg-slate-900 text-white rounded-2xl hover:bg-teal-600 disabled:opacity-50 transition-all shadow-xl"
            >
